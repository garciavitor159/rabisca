...
if ($acao === "editar_usuario") {
    ...
    try {
        $sql = "CALL proc_editar_usuario(?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id, $nome, $email, $senha, $pergunta_seguranca, $resposta]);
        $_SESSION["nome"] = $nome;
        retornar_resposta(true, [], "Dados editados com sucesso.", 201);
        ...
        retornar_resposta(false, [], "Erro: Não foi possível editar os seus dados.", 500);
    }
}
